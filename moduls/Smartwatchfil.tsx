...
// 🔹 Smartwatch ինտերֆեյս Firestore-ից եկող տվյալների համար
...
export default function Smartwatchfil() {
  ...
  const [watch, setWatch] = useState<Smartwatch[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data: Smartwatch[] = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...(doc.data() as Omit<Smartwatch, "id">),
        }));
        setWatch(data);
      } catch (err) {
