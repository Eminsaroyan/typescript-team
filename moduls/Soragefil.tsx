...
interface StoogeItem {
  ...
  anun: string;
  nkar: string;
  gin: string;   // Firestore-ում string է, եթե համարներ ես պահում կարող ես փոխել number-ի
  text: string;
  aparik: string;
}
    ...
    const fetchData = async () => {
      try {
        ...
        const data: StoogeItem[] = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...(doc.data() as Omit<StoogeItem, "id">),
        }));
        setStooge(data);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="mt-[20px] max-w-[1230px] mx-auto flex bg-white py-6">
        {/* Ձախ ֆիլտրերի UI */}
        <div className="mr-[10px] w-[288px]">
          <div className="bg-[#f8f9f9] border-[#ebedef] p-[20px] mb-[20px]">
            <h2 className="text-[18px] font-semibold mb-[10px]">Ֆիլտրեր</h2>
