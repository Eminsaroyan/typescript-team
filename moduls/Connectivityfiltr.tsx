...
// Տվյալների ինտերֆեյս Firestore-ից ստանալու համար
interface MiacumItem {
    id: string;
    anun: string;
    nkar: string;
    gin: string; // հիմա string է
    text: string;
    aparik: string;
}
...
export default function Connectivityfiltr() {
    ...
    const [miacum, setMiacum] = useState<MiacumItem[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const items: MiacumItem[] = snapshot.docs.map((doc) => {
                    const d = doc.data() as Omit<MiacumItem, "id">;
                    return {
                        id: doc.id,
                        ...d,
                        gin: String(d.gin), // ապահովություն, որ միշտ string լինի
                    };
                });
                setMiacum(items);
            } catch (error) {
                    ...
                    <div className="w-[288px] border-none">
                        <div className="mb-[20px] pt-[18px] bg-[#f8f9f9] border-[#ebedef]">
                            <div className="pt-[18px] border border-[#ebedef] bg-[#f8f9f9] mb-[20px]">
                                ...
                                <div className="pt-[18px] border border-[#ebedef] bg-[#f8f9f9] max-w-[320px] w-full">
                                    <button
                                        onClick={() => setTesakOpen(!isTesakOpen)}
                                        className="w-full text-start py-[10px] pr-[30px] pl-[10px] text-[18px] font-semibold tracking-[1px] border-none bg-[#ffffff]"
                                    >
                                        Տեսակ
                                    </button>
