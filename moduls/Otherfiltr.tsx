...
// Firestore-ից ստացվող տվյալների ինտերֆեյս
interface MyusItem {
  id: string;
  anun: string;
  nkar: string;
...
export default function Otherfiltr() {
  ...
  const [istesak, setTesakOpen] = useState<boolean>(true);
  const [islicqsOpen, setlicqOpen] = useState<boolean>(true);
  const [isusbOpen, setusb] = useState<boolean>(true);
  const [myus, setMyus] = useState<MyusItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data: MyusItem[] = snapshot.docs.map((doc) => {
          const d = doc.data() as Omit<MyusItem, "id">;
          return {
            id: doc.id,
            ...d,
          };
          ...
          <div className="w-[288px] border-none">
            <div className="mb-[20px] pt-[18px] bg-[#f8f9f9] border-[#ebedef]">
              <div className="pt-[18px] border border-[#ebedef] bg-[#f8f9f9] mb-[20px]">
                ...
                <div className="pt-[18px] border border-[#ebedef] bg-[#f8f9f9] max-w-[320px] w-full">
                  <button
                    onClick={() => setusb(!isusbOpen)}
                    className="w-full text-start py-[10px] pr-[30px] pl-[10px] text-[18px] font-semibold tracking-[1px] border-none bg-[#ffffff]"
                    aria-expanded={isusbOpen}
                  >
                    USB բնիկների քանակը
                  </button>
                  {isusbOpen && (
                    <div className="bg-white px-[20px] py-[15px] rounded-[12px] mt-[10px]">
                      {[2].map((opt, idx) => (
                        <label
                          ...
                          className="block cursor-pointer text-[19px] mb-[22px] text-[#2c3843] hover:text-blue-600"
                        >
                          <input type="checkbox" name="usb" value={opt} className="mr-[10px] accent-blue-500" />
                          {opt}
                        </label>
                      ))}
                    </div>
                ...
                <div className="pt-[18px] border border-[#ebedef] bg-[#f8f9f9] max-w-[320px] w-full">
                  <button
                    onClick={() => setlicqOpen(!islicqsOpen)}
                    className="w-full text-start py-[10px] pr-[30px] pl-[10px] text-[18px] font-semibold tracking-[1px] border-none bg-[#ffffff]"
                    aria-expanded={islicqsOpen}
                  >
                    Արագ լիցքավորում
                  </button>
                  {islicqsOpen && (
                ...
                <div className="pt-[18px] border border-[#ebedef] bg-[#f8f9f9] max-w-[320px] w-full">
                  <button
                    onClick={() => setTesakOpen(!istesak)}
                    className="w-full text-start py-[10px] pr-[30px] pl-[10px] text-[18px] font-semibold tracking-[1px] border-none bg-[#ffffff]"
                    aria-expanded={istesak}
                  >
                    Տեսակ
                  </button>
                  {istesak && (
